...
(def dom-cases
  ["document.getElementById('ws-proof').textContent = 'done';"
   ...
   ;; workers, sockets, and the property form of addEventListener
   "var w = new Worker('/w.js');"
   "var s = new WebSocket('ws://x');"
   "var w = new Worker('/w.js'); w.postMessage('hi');"
   "var s = new WebSocket('ws://x'); s.send('data');"
   "var w = new Worker('/w.js'); w.onmessage = function (e) { };"
...
(def ^:private quickjs-dom-shim
  ...
  (str "var __snap = " (js/JSON.stringify (clj->js dom-snapshot)) ";"
       ...
       "  });"
       "  Object.defineProperty(o, 'onclick', { set: function (fn) {"
       "      var k = __hs.length; __hs.push(fn);"
       "      __fx += __f('addEventListener') + __f(id)"
       "            + __f(__f('click') + __f(String(k))); } });"
       "  return o;"
       ...
       "var __hs = [];"
       "function Worker(url) {"
       "  var n = __hs.length; __hs.push(null);"
       "  __fx += __f('newWorker') + __f('#window') + __f(__f(url) + __f(String(n)));"
       "  var o = { postMessage: function (m) {"
       "      __fx += __f('postMessage') + __f('#wrk' + n) + __f(String(m)); } };"
       "  Object.defineProperty(o, 'onmessage', { set: function (fn) {"
       "      var k = __hs.length; __hs.push(fn);"
       "      __fx += __f('addEventListener') + __f('#wrk' + n)"
       "            + __f(__f('message') + __f(String(k))); } });"
       "  return o;"
       "}"
       "function WebSocket(url) {"
       "  var n = __hs.length; __hs.push(null);"
       "  __fx += __f('newWebSocket') + __f('#window') + __f(__f(url) + __f(String(n)));"
       "  var o = { send: function (m) {"
       "      __fx += __f('send') + __f('#sock' + n) + __f(String(m)); } };"
       "  Object.defineProperty(o, 'onmessage', { set: function (fn) {"
       "      var k = __hs.length; __hs.push(fn);"
       "      __fx += __f('addEventListener') + __f('#sock' + n)"
       "            + __f(__f('message') + __f(String(k))); } });"
       "  return o;"
       "}"
       "var __reqs = [];"
