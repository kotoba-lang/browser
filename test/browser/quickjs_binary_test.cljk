...
(deftest quickjs-binary-loader-accepts-wasm-magic
  (let [tmp (java.io.File/createTempFile "quickjs" ".wasm")
        ...
        adapter (quickjs/new-adapter {:origin "kotoba://shell" :profile-id "default"})
        attached (binary/attach-binary (binding/empty-binding adapter) loaded)]
...
(deftest quickjs-binary-loader-rejects-non-wasm
  (let [adapter (quickjs/new-adapter {:origin "kotoba://shell" :profile-id "default"})
        attached (binary/attach-binary (binding/empty-binding adapter)
...
(deftest quickjs-binary-integrity-is-checked-before-attach
  (let [bytes (byte-array [(byte 0) (byte 97) (byte 115) (byte 109) (byte 1)])
        ...
        ok (binary/attach-binary binding binary {:expected-sha256 (:quickjs.binary/sha256 binary)})
        bad (binary/attach-binary binding binary {:expected-sha256 "bad"})]
