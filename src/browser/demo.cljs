...
(defn sample-html
  ...
  (str
   ...
   "<section id=\"negative-z-index-proof\" style=\"display:flex; flex-direction:column; gap:8\">"
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "Below: a position:absolute box with z-index:-1 fully overlaps an "
   "in-flow sibling that fully covers it back. Previously a negative "
   "z-index still painted LAST (on top) regardless of sign, so the "
   "absolutely-positioned box would incorrectly hide the in-flow one. "
   "You should see ONLY the teal in-flow box -- the orange positioned "
   "box is fully behind it, not visible anywhere."
   "</p>"
   "<div style=\"position:relative; width:220; height:60; background:#16202f\">"
   "<div id=\"negz-box\" style=\"position:absolute; top:0; left:0; width:220; height:60; background:#e0a458; z-index:-1\">orange (should be hidden)</div>"
   "<div id=\"in-flow-box-covering-negz\" style=\"width:220; height:60; background:#4fd1c5; color:#121724\">teal (should fully cover orange)</div>"
   "</div>"
   "</section>"
   "<section id=\"position-fixed-proof\" style=\"display:flex; flex-direction:column; gap:8\">"
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "Two boxes below: the first is position:fixed (right:8), the second an "
   "ordinary static sibling. Previously position:fixed was not recognized "
   "at all and behaved exactly like position:static -- the fixed box "
   "would still occupy layout space and push the second box down instead "
   "of both rendering at the SAME top-aligned y, side by side."
   "</p>"
   "<div style=\"position:relative; width:220; height:60; background:#16202f; border-width:2; border-color:#4fb3a6\">"
   "<div id=\"fixed-box\" style=\"position:fixed; right:8; width:90; height:30; background:#e0a458; color:#121724\">fixed box</div>"
   "<div id=\"static-box-after-fixed\" style=\"width:90; height:30; background:#4fd1c5; color:#121724\">static box</div>"
   "</div>"
   "</section>"
   "<section id=\"inherit-keyword-proof\" style=\"display:flex; flex-direction:column; gap:8\">"
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "The parent below is teal; its child explicitly writes color: inherit "
   "-- a real, extremely common CSS idiom. Previously the literal string "
   "\"inherit\" was stored as the child's own color, which no color "
   "parser recognizes, so the child's text silently painted fully "
   "transparent (invisible) instead of matching its parent's teal."
   "</p>"
   "<div id=\"inherit-parent\" style=\"color:#4fd1c5; background:#16202f; padding:8\">"
   "Parent text (teal)"
   "<div id=\"inherit-child\" style=\"color:inherit\">Child text (should also be teal, not invisible)</div>"
   "</div>"
   "</section>"
   "<section id=\"align-items-stretch-proof\" style=\"display:flex; flex-direction:column; gap:8\">"
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "The two boxes below share a flex row with NO align-items declared -- "
   "real CSS's own default is stretch, so the left box (which authors no "
   "height of its own) should visually match the right box's explicit "
   "40px height. Previously align-items:stretch was never implemented as "
   "a size change at all -- only as a position offset -- so the left box "
   "stayed at its own tiny natural content height instead."
   "</p>"
   ...
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "A flex-wrap:wrap row below, narrow enough that the three boxes wrap "
   "onto two lines -- align-items:center. Previously wrapped rows ignored "
   "align-items entirely (not even the pre-existing center/flex-end "
   "offsets, let alone stretch), so every box stayed pinned to its own "
   "row's top edge regardless of this container's own align-items value. "
   "The shorter blue box on the first line should now sit vertically "
   "centered against its taller teal row-mate."
   "</p>"
   "<div style=\"display:flex; flex-wrap:wrap; align-items:center; gap:8; width:200\">"
   "<div style=\"width:80; height:20; background:#5b8dd6\"></div>"
   "<div style=\"width:80; height:60; background:#4fb3a6\"></div>"
   "<div style=\"width:80; height:30; background:#e0a458\"></div>"
   "</div>"
   "</section>"
   "<section id=\"box-shadow-script-mutation-proof\" style=\"display:flex; flex-direction:column; gap:8\">"
   "<p style=\"color:#9fb0c9; font-size:13\">"
   "The gray box below gets a real box-shadow set by SCRIPT, one "
   "individual style property at a time (el.style.boxShadowX = ...), not "
   "authored as a single box-shadow: ... declaration. Previously this "
   "specific script-driven path left the shadow's offset/spread as raw, "
   "uncoerced strings, corrupting the shadow's own on-screen position "
   "into garbage coordinates (or crashing outright, depending on "
   "runtime) instead of the real 10px-right/10px-down shadow below."
   "</p>"
   "<div id=\"box-shadow-script-target\" style=\"width:80; height:40; background:#5b8dd6\"></div>"
   "</section>"
   "<script>"
   "var bsmTarget = document.getElementById('box-shadow-script-target');"
   "bsmTarget.style.boxShadowX = '10';"
   "bsmTarget.style.boxShadowY = '10';"
   "bsmTarget.style.boxShadowColor = 'rgba(0,0,0,0.6)';"
   "</script>"
   ...
   "stale forever. The 'before' value below also proves a separate, "
   "later-fixed bug: a bare checked attribute (no =value at all, as "
   "written below) now correctly reads back as the empty string, not "
